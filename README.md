...
## Règles des jeux\n\nLa commande `/rules jeu:<jeu>` affiche les règles détaillées et les gains possibles pour chaque jeu.\n\nDes raccourcis sont également disponibles : `/blackjack-rules`, `/craps-rules`, `/coinflip-rules`, `/dice-rules`, `/slots-rules`, `/roulette-rules`, `/rps-rules`, `/higherlower-rules` et `/hilo-rules`.\n\nHilo se joue avec `/hilo mise`, puis `/hilo-guess choix:<plus|moins>`. La cote augmente selon la probabilité restante (x1,5, x2 ou x2,5), est plafonnée à x10 et peut être encaissée avec `/hilo-cashout`. L As est la carte la plus basse ; les choix sont inférieur ou égal / supérieur ou égal, donc une carte de même valeur valide le tour.\n## Données et déploiement
